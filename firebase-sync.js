...
const trialExpired = createdAt
  ? (Date.now() - parseInt(createdAt, 10)) / (1000 * 60 * 60 * 24) > 30 && !devMode
  : false;
...
if (trialExpired) {
  ...
  window.FINORA_AUTH = {
    ...
    // ✅ 新增補回的功能
    addAccount: async (displayName) => {
      const id = await createNewAccount(displayName);
      localStorage.setItem("selectedAccount", id);
      return id;
    },

    renameAccount: async (newName) => {
      const accountId = localStorage.getItem("selectedAccount");
      await setAccountDisplayName(accountId, newName);
    }
  };
}
