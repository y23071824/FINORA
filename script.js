...
let assets = JSON.parse(localStorage.getItem("assets") || "[]"); let bankHistory = JSON.parse(localStorage.getItem("banks") || "[]"); let editIndex = null; let exchangeRates = {};

async function fetchExchangeRates() { try { const res = await fetch("https://api.exchangerate.host/latest?base=TWD&symbols=USD,JPY,EUR"); const data = await res.json(); exchangeRates = data.rates; localStorage.setItem("exchangeRates", JSON.stringify(exchangeRates)); } catch (e) { console.error("匯率載入失敗", e); } }

async function fetchStockPrice(symbol, category) { try { if (category === "台股") { symbol += ".TW"; const res = await fetch(https://query1.finance.yahoo.com/v7/finance/quote?symbols=${symbol}); const data = await res.json(); return data.quoteResponse.result[0]?.regularMarketPrice || null; } else { const apiKey = "de909496c6754a89bc33db0306c2def8"; const res = await fetch(https://api.twelvedata.com/price?symbol=${symbol}&apikey=${apiKey}); const data = await res.json(); return data.price ? parseFloat(data.price) : null; } } catch (e) { console.error("查價失敗", e); return null; } }

document.getElementById("stock-symbol")?.addEventListener("blur", async () => { const symbolInput = document.getElementById("stock-symbol").value.trim().toUpperCase(); const category = document.getElementById("stock-category").value; if (!symbolInput) return; const price = await fetchStockPrice(symbolInput, category); if (price !== null) { document.getElementById("price").value = price; } else { alert("查無此股票代碼或查價失敗，請重新確認。"); } });
...
function render() { assetList.innerHTML = ""; totalsList.innerHTML = ""; const totals = {}, profits = {}, toTWD = {};
...
assets.forEach((item, index) => {
  ...
  if (item.type === "股票") {
    ...
    extra = `<br>保單名稱：${item.policyName}<br>保額：$${item.policyAmount}, 年期：${item.policyYears}, 年繳保費：$${item.policyPremium}`;
  } else {
  ...
  toTWD[currency] = toTWD[currency] || (currency === "TWD" ? 1 : exchangeRates[currency] || 0);

  const li = document.createElement("li");
  li.innerHTML = `${item.currency} (${item.bank}) ${item.note || ""}${extra}<div class="button-group"><button onclick="editAsset(${index})">編輯</button><button onclick="deleteAsset(${index})">刪除</button></div>`;
...
for (const ccy in totals) {
  ...
  const rate = toTWD[ccy];
  const li = document.createElement("li");
  li.innerHTML = `${ccy}: $${(total + profit).toLocaleString()}（盈餘：$${profit.toLocaleString()}）` +
    (rate ? ` ≈ NT$${((total + profit) * rate).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : "");
