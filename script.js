...
async function fetchExchangeRates() { try { const res = await fetch("https://api.exchangerate.host/latest?base=USD&symbols=TWD,JPY,EUR"); const data = await res.json(); localStorage.setItem("exchangeRates", JSON.stringify(data.rates)); } catch (e) { console.error("匯率載入失敗", e); } }
...
document.getElementById("stock-category")?.addEventListener("blur", async () => { const symbol = document.getElementById("stock-category").value.trim(); if (symbol) { const price = await fetchStockPrice(symbol); if (price) document.getElementById("price").value = price; } });

document.getElementById("currency")?.addEventListener("change", () => { const currency = document.getElementById("currency").value; const rates = JSON.parse(localStorage.getItem("exchangeRates") || "{}"); if (currency === "USD" || currency === "JPY" || currency === "EUR") { alert(目前 ${currency} 對 TWD 匯率：約 ${rates[currency] || "查詢中"}); } });
