...
// 當頁面載入完成後執行 document.addEventListener("DOMContentLoaded", () => { const form = document.getElementById("asset-form"); const typeSelect = document.getElementById("type"); const stockFields = document.getElementById("stock-fields"); const insuranceFields = document.getElementById("insurance-fields"); const amountField = document.getElementById("amount-field"); const assetList = document.getElementById("asset-list"); const totalsList = document.getElementById("totals-list"); const profitList = document.getElementById("stock-profit-list"); const bankDatalist = document.getElementById("bank-list");

let assets = JSON.parse(localStorage.getItem("assets") || "[]"); let bankHistory = JSON.parse(localStorage.getItem("banks") || "[]"); let editIndex = null;

// 匯率與股價 API async function fetchExchangeRates() { try { const res = await fetch("https://api.exchangerate.host/latest?base=USD&symbols=TWD,JPY,EUR"); const data = await res.json(); localStorage.setItem("exchangeRates", JSON.stringify(data.rates)); } catch (e) { console.error("匯率載入失敗", e); } }

async function fetchStockPrice(symbol) { try { const res = await fetch(https://query1.finance.yahoo.com/v7/finance/quote?symbols=${symbol}); const data = await res.json(); const quote = data.quoteResponse.result[0]; return quote?.regularMarketPrice || null; } catch (e) { console.error("股價查詢失敗", e); return null; } }

// 自動帶入股價（從股票代碼欄位） document.getElementById("stock-symbol")?.addEventListener("blur", async () => { const symbol = document.getElementById("stock-symbol").value.trim(); if (symbol) { const price = await fetchStockPrice(symbol); if (price) document.getElementById("price").value = price; } });

// 幣別匯率提示 document.getElementById("currency")?.addEventListener("change", () => { const currency = document.getElementById("currency").value; const rates = JSON.parse(localStorage.getItem("exchangeRates") || "{}"); if (["USD", "JPY", "EUR"].includes(currency)) { alert(目前 ${currency} 對 TWD 匯率：約 ${rates[currency] || "查詢中"}); } });
...
function render() { assetList.innerHTML = ""; totalsList.innerHTML = ""; profitList.innerHTML = "";
